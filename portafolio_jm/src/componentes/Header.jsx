...
export const Header = () => {
  ...
  const linkInactive = 'text-gray-400 hover:text-gray-200';
  const linkActive =
    'bg-white/10 text-gray-200 shadow-[0_0_12px_-6px_rgba(139,92,246,0.6)]';

  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-[#0e0e1c]/80 backdrop-blur-md border-b border-white/10 shadow-md'
      ...
      >
        <ul className="flex flex-col items-center gap-2 bg-[#0e0e1c]/80 backdrop-blur-md border border-white/10 rounded-2xl py-4 text-sm font-medium text-gray-400">
          {sections.map(({ id, label }) => (
            <li key={id}>
              <a
                href={`#${id}`}
